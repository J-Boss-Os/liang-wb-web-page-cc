---
name: refactor-page-anytrack
description: 将anytrack的落地页重构为Thymeleaf模板，仅提取logo、品牌名、CTA链接、产品主图四个核心字段
...
- 生成`lp_config.json`：
    ...
    - `type`：图片用`IMAGE`，跳转链接用`URL`，文本用`TEXT`
...
- 拷贝一份html为`original.html`，将原html转为Thymeleaf模板,替换的内容就是根据`lp_config.json`文件和`产品映射table表`中对应的`x-path`dom上：
    - 图片替换：`th:src="${key}"`（删除所有图片相关标签上的响应式图片候选属性如srcset，但需保留th:src、src、width、height、class、alt 等基础属性）
    - 品牌替换：`th:text="${key}"`
